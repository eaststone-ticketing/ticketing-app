...
const R2_BUCKET = "ticketing-images"
...
app.get("/arendepdf/:arendeId", authenticateToken, async (req, res) => {
  try {
    ...
    if (arende.arendeTyp === "Ny sten" && !kyrkogardar_stockholm.includes(arende.kyrkogard) && arende.kyrkogard !== "AAA") {
      formName = "form.pdf"
    ...
    if (formName === "form_sthlm_new1.pdf"){
      ...
      form.getTextField("Typsnitt m.m").setText(`${arende.färg} ${arende.typsnitt} ${arende.forsankt}`)
    }
    }
