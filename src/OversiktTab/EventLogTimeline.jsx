...
function DescriptiveStatistics({ arr }) {
  if (!arr) {
    return <p>Ingen variabel vald</p>
  }

  return <div className="data-view-stats">
    <p><strong>Medelvärde: </strong>{formatNumber(average(arr))}</p>
    <p><strong>Standardavvikelse: </strong>{formatNumber(stdev(arr))}</p>
    <p><strong>Antal (n): </strong>{arr.length}</p>
  </div>
}
  ...
  return <div>
    ...
    <div>
      {fields.map((field) => <button
        key={field}
        className={`data-filter-chip ${command.includes(field) ? "active" : ""}`}
        onClick={() => {
...
export default function EventLogTimeline(){
    ...
    return <div className = "data-view">
        ...
        {arr === "price" && <div>
          <DescriptiveStatistics arr = {producePriceList(arenden,typ,status,ursprung)} />
        ...
        {(arr === "timeToFinish" || arr === "timeToGraveyardApproval") && <div>
          <DescriptiveStatistics arr = {produceTimeSpanList(arenden, kyrkogard, traces, arr)} />
